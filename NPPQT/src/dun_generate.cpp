 ...
 */
static void grid_queue_create(grid_queue_type *q, size_t max_size)
{
 ...
 */
static void grid_queue_destroy(grid_queue_type *q)
{
 ...
 */
static bool grid_queue_push(grid_queue_type *q, byte y, byte x)
{
    /* Check space */
 ...
 */
static void grid_queue_pop(grid_queue_type *q)
{
    /* Something to pop? */
...
static void build_type_lesser_vault(int y0, int x0)
{
    ...
    /* Message */
    if (cheat_room) message(QString("Lesser vault (%1)") .arg(v_ptr->vault_name));
...
static void mark_g_vault(int y0, int x0, int hgt, int wid)
{
    ...
    grid_queue_type queue;
    grid_queue_type *que_ptr = &queue;
...
static void build_type_greater_vault(int y0, int x0)
{
    ...
    /* Message */
    if (cheat_room) message(QString("Greater vault (%1)") .arg(v_ptr->vault_name));
...
static void build_type_special_vault(int y0, int x0)
{
    ...
    /* Message */
    if (cheat_room) message(QString("Special vault (%1)") .arg(v_ptr->vault_name));
...
static int fractal_map_is_connected(fractal_map map, fractal_template *t_ptr)
{
    ...
    /* Find a floor grid */
    for (y = 0; (y < t_ptr->size) && GRID_QUEUE_EMPTY(que_ptr); y++)
    {
        for (x = 0; (x < t_ptr->size) && GRID_QUEUE_EMPTY(que_ptr); x++)
        {
            /* Found one */
            if (map[y][x] >= FRACTAL_FLOOR)
            {
                /* Put it on the queue */
                grid_queue_push(que_ptr, y, x);
    ...
    while (!GRID_QUEUE_EMPTY(que_ptr))
    {
        ...
        for (i = 0; i < 8; i++)
        {
            ...
            if (!grid_queue_push(que_ptr, yy, xx)) continue;
